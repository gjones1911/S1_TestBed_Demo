...
def set_label_from_folder(folder_name, mapping):
    ...
    code_value = mapping.get(fault_key, "unknown")
    print(f"code value is {code_value} for folder_name: {folder_name}")
    return code_value

def merge_sensor_data(directory):
    """
    Reads multiple sensor data files from subdirectories, merges per fault folder, and concatenates them.
    """
    fault_mapping = {
        "baseline": 0,
        "bent_shaft": 1,
        ...
        for file in files:
            if file.endswith(".csv"):
                ...
                if fault_df is None:
                    ...
                    fault_df = pd.merge(fault_df, df, on="timestamp", how="inner")
...
if __name__ == "__main__":
    ...
    output_file = output_dir + "/merged_sensor_data_2025_a.csv"
